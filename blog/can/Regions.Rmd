...
regions<-regions %>% mutate(Accurate_Episode_Date=ymd(Accurate_Episode_Date)) %>% 
  select(FSA, Accurate_Episode_Date) 
...
plot_project<-function(country="Ontario",conf=20,method=c("Estimate_R","Simulate_R"),
  ...
  out <- suppressMessages(
    ...
    labs(title= paste(titlab,"of observed and predicted confirmed cases, ",df$iso3c),
         ...
         subtitle = paste("Data as of", format(max(df$dates), "%A, %B %e, %Y")))+
...
write.csv(result$temp,file = "Ontario.csv")
write.csv(central$temp,file = "Ontario.csv")
write.csv(east$temp,file = "Ontario.csv")
write.csv(north$temp,file = "Ontario.csv")
write.csv(toronto$temp,file = "Ontario.csv")
write.csv(west$temp,file = "Ontario.csv")
```

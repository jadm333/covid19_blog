...
deaths_by_country = list()
  ...
  for (ii in 1:ncol(covariates1)) {
    ...
    d1[covariate] <- (as.Date(d1$date, format='%d/%m/%Y') >= as.Date(covariates1[1,covariate]))*1  # should this be > or >=?
  }
...
fit = sampling(m,data=stan_data,iter=10000,warmup=1000,chains=4,thin=4,control = list(adapt_delta = 0.90, max_treedepth = 10))
